...
/*check whether current edge can be treat as a seed*/
{
	...
	/*	if ( isInUberSet(bb[i]->genes,e->gene_one) && isInUberSet(bb[i]->genes, e->gene_two) )*/
			return FALSE; 
	...
	/*	if ( isInUberSet(bb[i]->genes, e->gene_one) ) */
		{ 
			fg = TRUE;
		       	break; 
		}
	...
	for ( i = 0; i < block_id; i++)
		...
		if ( isInUberSet(bb[i]->genes, e->gene_two) ) 
		{ 
			fg = TRUE; 
			break; 
		}
                     ...
                     int *components, struct dyStack *allincluster, long double *pvalues)
{
	...
	for (i = 0; i < rows; i++)
	{
		...
		uber_row = uber_score(dsItem(genes,0),i)* uber_ratio *arr_rows[i];
		uber_row1 = uber_score(dsItem(genes,1),i)* uber_ratio *arr_rows[i];
		arr_rows_b[i] = arr_rows[i] + MAX(uber_row,uber_row1);
	}
	...
	while (*components < rows)
	{
		...
		for (i=0; i< rows; i++)
		{
			...
			uber_row = uber_score(dsItem(genes,0), i) * uber_ratio *arr_rows[i];
			uber_row1 = uber_score(dsItem(genes,1),i)* uber_ratio *arr_rows[i];
		    cnt_all = cnt_all + cnt + MAX(uber_row,uber_row1);
...
/************************************************************************/
/* caculate if a new gene in a block */
int uber_row_score(int ki,int a[],struct dyStack *genes)
{ 
    int i;
    for (i=0; i< genes->top + 1;i++)
        if (gene_uber[ki] == a[i])
            return 1;
        else if (gene_uber[ki] == 0)
            return 0;
    return -1; 
}
/************************************************************************/
...
int cluster (FILE *fw, Edge **el, int n)
{
	...
	{	
		...
		/*	for (j=0; j<dsSize(allincluster); j++)
	        { 
	            printf("%d ",allincluster->items[j]);
	        }
	        printf("\n");
		   if(isInUberSet(allincluster,e->gene_one) && isInUberSet(allincluster,e->gene_two)) */
				flag = FALSE;
		...
		int m_cnt;
		int *uber_block;
		uber_block = (int*)malloc(sizeof(int)*(genes->top + 1));
		/* int uber_block[genes->top + 1];*/
		int i; 
		/*printf("add some new possible genes\n");*/
	    fflush(stdout);
		for(i = 0; i < genes->top + 1; i ++ )
		{
		    uber_block[i] = gene_uber[genes->items[i] + 1];
		} 
		/*printf ("begin add genes");*/

		for ( ki = 0; ki < rows; ki++)
		{
			if (po->IS_list && !sublist[ki]) continue;
			/* printf("%d \n", uber_row_score(ki,uber_block,genes));*/
			m_cnt = intersect_row(colcand, arr_c[dsItem(genes,0)], arr_c[ki]); 
			m_cnt = m_cnt + uber_row_score(ki,uber_block,genes) * uber_ratio *m_cnt;
			/* printf ("%d expansion genes\n",ki);*/
		...
		/* printf("add some new negative possible genes\n");*/
	    fflush(stdout);
	...
	/* printf("The number of true seed is %d",h); */
...
static void print_params(FILE *fw)
{
	...
	fprintf(fw, "# IDAM version %.1f output\n", VER);
	fprintf(fw, "# Datafile %s: %s type\n", po->FN, filedesc);
	fprintf(fw, "# Parameters: -k %d -f %.2f -o %d",
			po->COL_WIDTH, po->FILTER, po->RPT_BLOCK);
	if (!po->IS_DISCRETE) 
